...
- `src/mcp_server.py` — регистрация MCP-инструментов (10 tools).
...
| `Ждать` без `Асинх` (BSL-ASYNC-001) | `Асинх Функция` в сигнатуре | 2026-07-08 |
| `Асинх` в серверном модуле (BSL-ASYNC-003) | Разделять на Клиент + Сервер модули | 2026-07-08 |
| `Перем ... Экспорт` в общем модуле (BSL-MODULE-VAR-001) | Stateless — состояние в форме | 2026-07-08 |
| `ЗаписьЖурналаРегистрации` на клиенте (CTX001) | BslContextChecker + get_method_details | 2026-07-08 |
| `Метаданные` на клиенте (CTX001) | Серверный вызов для проверки метаданных | 2026-07-08 |

---

## Генерация BSL-кода — ОБЯЗАТЕЛЬНЫЙ WORKFLOW (B8)

**Перед генерацией ЛЮБОГО BSL-кода:**

1. **Определи целевой контекст** (клиент/сервер/мобильное)
2. **Для каждого метода платформы** — вызови `get_method_details(<имя>)`:
   - Проверь `availability` — доступен ли в целевом контексте?
   - Проверь `version_since` — доступен ли в целевой версии?
   - Проверь `version_deprecated` — не устарел ли?
3. **После генерации** — вызови `check_bsl_context(code, target_context)`
4. Если есть ERROR — исправь и повтори

**Методы, НЕдоступные на клиенте** (частые ошибки):
- `ЗаписьЖурналаРегистрации` → только сервер
- `УровеньЖурналаРегистрации` → только сервер
- `Метаданные` → только сервер
...
**CLI для проверки:**
```bash
1c-ai platform search "ЗаписьЖурналаРегистрации"   # поиск метода
1c-ai platform method ЗаписьЖурналаРегистрации      # полная карточка
1c-ai platform check module.bsl --context thin_client  # проверка кода
1c-ai platform versions                              # список версий
```
