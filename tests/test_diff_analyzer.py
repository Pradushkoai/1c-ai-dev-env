...
from diff_analyzer import DiffAnalyzer, ConfigDiff, ObjectChange
...
class TestAddedObjects:
    def test_added_catalog_detected(self, analyzer):
        ...
        new = {'objects': {'Catalogs': [{'name': 'Старый', 'child_objects': {}}, {'name': 'Новый', 'child_objects': {}}]}}
...
class TestRemovedObjects:
    def test_removed_catalog_detected(self, analyzer):
        old = {'objects': {'Catalogs': [{'name': 'Удалённый', 'child_objects': {}}, {'name': 'Остался', 'child_objects': {}}]}}
...
class TestModifiedObjects:
    def test_added_attribute_detected(self, analyzer):
        old = {'objects': {'Catalogs': [{'name': 'Тест', 'child_objects': {'attributes': [{'name': 'Рекв1', 'types': []}]}}]}}
        new = {'objects': {'Catalogs': [{'name': 'Тест', 'child_objects': {'attributes': [{'name': 'Рекв1', 'types': []}, {'name': 'Рекв2', 'types': []}]}}]}}
        diff = analyzer.compare_data(old, new)
    ...
    def test_removed_attribute_detected(self, analyzer):
        old = {'objects': {'Catalogs': [{'name': 'Тест', 'child_objects': {'attributes': [{'name': 'Рекв1'}, {'name': 'Рекв2'}]}}]}}
    ...
    def test_no_changes_when_identical(self, analyzer):
        old = {'objects': {'Catalogs': [{'name': 'Тест', 'synonym': 'Синоним', 'child_objects': {'attributes': [{'name': 'А'}], 'forms': [{'name': 'Ф'}]}}]}}
        new = {'objects': {'Catalogs': [{'name': 'Тест', 'synonym': 'Синоним', 'child_objects': {'attributes': [{'name': 'А'}], 'forms': [{'name': 'Ф'}]}}]}}
        diff = analyzer.compare_data(old, new)
