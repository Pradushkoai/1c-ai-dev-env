...
class TestNoParameters:
    def test_no_params_detected(self, checker):
        schema = {'name': 'Test', 'parent_name': 'Test', 'schema': {'data_sets': [{'query': 'ВЫБРАТЬ * ИЗ Таблица'}], 'parameters': []}}
        issues = checker.check_schema(schema)
        assert any(i.rule_id == 'SKD001' for i in issues)

    def test_with_params_ok(self, checker):
        schema = {'name': 'Test', 'parent_name': 'Test', 'schema': {'data_sets': [{'query': 'ВЫБРАТЬ * ИЗ Таблица'}], 'parameters': [{'name': 'Период', 'types': ['xs:dateTime']}]}}
        issues = checker.check_schema(schema)
...
class TestNoFilters:
    def test_no_filters_detected(self, checker):
        schema = {'name': 'Test', 'parent_name': '', 'schema': {'data_sets': [], 'parameters': [{'name': 'P', 'types': ['xs:string']}], 'filters': []}}
        issues = checker.check_schema(schema)
...
class TestNoTotalFields:
    def test_no_totals_detected(self, checker):
        schema = {'name': 'Test', 'parent_name': '', 'schema': {'data_sets': [{'fields': [{'data_path': 'Сумма'}], 'query': 'ВЫБРАТЬ Сумма'}], 'total_fields': []}}
        issues = checker.check_schema(schema)
...
class TestOverloadedSKD:
    def test_too_many_fields_detected(self, checker):
        ...
        schema = {'name': 'Big', 'parent_name': '', 'schema': {'data_sets': [{'fields': fields}], 'parameters': [{'name': 'P', 'types': ['xs:string']}], 'total_fields': [{'data_path': 'Сумма'}], 'filters': [{'name': 'F'}]}}
        issues = checker.check_schema(schema)
...
class TestQueryNoWhere:
    def test_query_no_where_detected(self, checker):
        schema = {'name': 'Test', 'parent_name': '', 'schema': {'data_sets': [{'query': 'ВЫБРАТЬ * ИЗ Документ.Реализация', 'fields': []}], 'parameters': [{'name': 'P', 'types': ['xs:string']}], 'total_fields': [{'data_path': 'S'}], 'filters': [{'name': 'F'}]}}
        issues = checker.check_schema(schema)
...
class TestParamNoType:
    def test_param_no_type_detected(self, checker):
        schema = {'name': 'Test', 'parent_name': '', 'schema': {'data_sets': [{'query': 'ВЫБРАТЬ * ИЗ Таблица ГДЕ Код = &Код'}], 'parameters': [{'name': 'Код', 'types': []}], 'total_fields': [{'data_path': 'S'}], 'filters': [{'name': 'F'}]}}
        issues = checker.check_schema(schema)
...
class TestEmptyQuery:
    def test_empty_query_detected(self, checker):
        schema = {'name': 'Test', 'parent_name': '', 'schema': {'data_sets': [{'query': '', 'fields': []}], 'parameters': [{'name': 'P', 'types': ['xs:string']}], 'total_fields': [{'data_path': 'S'}], 'filters': [{'name': 'F'}]}}
        issues = checker.check_schema(schema)
...
class TestNoDataSource:
    def test_no_data_source_detected(self, checker):
        schema = {'name': 'Test', 'parent_name': '', 'schema': {'data_sources': [], 'data_sets': [], 'parameters': [{'name': 'P', 'types': ['xs:string']}], 'total_fields': [{'data_path': 'S'}], 'filters': [{'name': 'F'}]}}
        issues = checker.check_schema(schema)
