...
class TestEmptyForm:
    ...
    def test_non_empty_form_ok(self, checker):
        form = {'name': 'TestForm', 'parent_name': 'Test', 'form': {'element_count': 1, 'items': [{'type': 'InputField', 'name': 'Поле1', 'data_path': 'Объект.Реквизит1'}]}}
        issues = checker.check_form(form)
...
class TestHiddenElements:
    def test_hidden_elements_detected(self, checker):
        items = [{'type': 'InputField', 'name': f'Поле{i}', 'data_path': f'Объект.Поле{i}', 'visible': False} for i in range(4)]
