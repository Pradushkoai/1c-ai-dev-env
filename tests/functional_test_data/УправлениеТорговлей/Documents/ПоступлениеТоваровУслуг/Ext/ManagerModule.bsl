...
Процедура ЗаполнитьПоОстаткамЗаказов(ДанныеОтбора,
	...
	Пока ВыборкаТовары.Следующий() Цикл
		...
		Исключение
		СтрокаТаб.КоличествоУпаковок = ВыборкаТовары.Количество;
		КонецПопытки; 
		//СтрокаТаб.КоличествоУпаковок = ВыборкаТовары.Количество / ВыборкаТовары.Коэффициент;
...
Процедура ЗаполнитьПоЗаказамПоПриемке(ДанныеОтбора,Товары,СкладПоступления,ПоОрдеру,МассивЗаказов = Неопределено) Экспорт
	...
	Если ПринятыеТовары.Количество() > 0 Тогда
		...
		СписокСвойств = "Номенклатура,Характеристика,Склад,ЗаказПоставщику";
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	// HS сумма в валюте документа для расчета себестоимости с учетом авансов
	|	,СУММА(ТаблицаТовары.СуммаСНДС) КАК HS_СуммаСНДСДок
	|	,СУММА(ТаблицаТовары.СуммаНДС) КАК HS_СуммаНДСДок
	...
	// HS сумма в валюте документа для расчета себестоимости с учетом авансов
	|	,0 КАК HS_СуммаСНДСДок
	|	,0 КАК HS_СуммаНДСДок
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора,
	...
	|	ТоварыНакладной.СуммаРучнойСкидки            КАК СуммаРучнойСкидки,
	// HS
	|	ТоварыНакладной.Цена            	 		 КАК ЦенаСоСкидкой,
