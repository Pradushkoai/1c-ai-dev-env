...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для ТекИндекс = 0 По Товары.Количество()-1 Цикл
		...
		//Если Товары[ТекИндекс].ВидТМЦ.ТипТМЦ = Перечисления.ТипыТМЦ.Семена Тогда          ////// 77w qaws
		//	Если Не ЗначениеЗаполнено(Товары[ТекИндекс].Партия_МССО) Тогда
		//		
		//		ТекстОшибки = НСтр("ru='Не заполнена колонка ""Партия МССО (партионная карточка)""'");
		//			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
		//			ТекстОшибки + АдресОшибки,
		//			ЭтотОбъект,
		//			ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", Товары[ТекИндекс].НомерСтроки, "Партия_МССО"),
		//			,
		//			Отказ);
		//		
		//	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		HS_ПартионныйУчет.ЗаполнитьСерии(ЭтотОбъект, Отказ, РежимЗаписи, РежимПроведения);
	...
	Для каждого Строка Из Товары Цикл   ////// 77w qaws
	

		Если ЗначениеЗаполнено(Строка.Партия_МССО) Тогда
			
			Вес1Места = 0;
			ВесНеттоПартии = 0;
			КоличествоМестПартии = 0;
			
			Попытка
				Вес1Места = Строка.Вес1Места;
				ВесНеттоПартии = Строка.ВесНеттоМССО;
				КоличествоМестПартии = Строка.КолГрузМест;
			Исключение
			    //ОписаниеОшибки()
			КонецПопытки; 
			
			Партия_МССО_Объект = Справочники.ПартионныеКарточки.НайтиПоКоду(Строка.Партия_МССО.Код).ПолучитьОбъект();
			
			//Если НЕ ЗначениеЗаполнено(Партия_МССО_Объект.ВесОдногоМеста) Тогда
			//	Партия_МССО_Объект.ВесОдногоМеста = Вес1Места;
			//КонецЕсли; 
			//
			//Если НЕ ЗначениеЗаполнено(Партия_МССО_Объект.ВесНетто) Тогда
			//	Партия_МССО_Объект.ВесНетто = ВесНеттоПартии;
			//КонецЕсли; 
			//
			//Если НЕ ЗначениеЗаполнено(Партия_МССО_Объект.КоличествоМест) Тогда
			//	Партия_МССО_Объект.КоличествоМест = КоличествоМестПартии;
			//КонецЕсли; 
			
			//Если НЕ ЗначениеЗаполнено(Партия_МССО_Объект.ВесОдногоМеста) Тогда
			//	//Партия_МССО_Объект.ВесОдногоМеста = Вес1Места;
			//	Партия_МССО_Объект.ВесОдногоМеста = Строка.Номенклатура.ВесЧислитель;
			//КонецЕсли; 
			//
			//Если НЕ ЗначениеЗаполнено(Партия_МССО_Объект.ВесНетто) Тогда
			//	//Партия_МССО_Объект.ВесНетто = ВесНеттоПартии;
			//	Партия_МССО_Объект.ВесНетто = Строка.Номенклатура.ВесЧислитель * Строка.КоличествоУпаковок;
			//КонецЕсли; 
			//
			//Если НЕ ЗначениеЗаполнено(Партия_МССО_Объект.КоличествоМест) Тогда
			//	//Партия_МССО_Объект.КоличествоМест = КоличествоМестПартии;
			//	Партия_МССО_Объект.КоличествоМест = Строка.КоличествоУпаковок;
			//КонецЕсли;
			//
			//Если НЕ ЗначениеЗаполнено(Партия_МССО_Объект.ДатаПриемногоАкта) Тогда
			//	 Партия_МССО_Объект.ДатаПриемногоАкта = ЭтотОбъект.Дата;
			//КонецЕсли; 
			//
			...
			//Если НЕ ЗначениеЗаполнено(Партия_МССО_Объект.ДатаУдостоверения) Тогда
			//	 Партия_МССО_Объект.ДатаУдостоверения = ЭтотОбъект.Дата;
			//КонецЕсли; 
			
			Партия_МССО_Объект.Наименование = СокрЛП(Строка.Номенклатура.Наименование);
			Партия_МССО_Объект.Номенклатура = Строка.Номенклатура.Ссылка;
			Партия_МССО_Объект.Поставщик    = ЭтотОбъект.Контрагент.Ссылка;
			
			Попытка
			    Если НЕ ЗначениеЗаполнено(Партия_МССО_Объект.УдостоверениеОКондиционности) Тогда
			    	Партия_МССО_Объект.УдостоверениеОКондиционности = Строка.Номенклатура.Удостоверение;
			    КонецЕсли; 
			  
			    Если НЕ ЗначениеЗаполнено(Партия_МССО_Объект.Свидетельство) Тогда
			    	Партия_МССО_Объект.Свидетельство = Строка.Номенклатура.ПаспортКачества;
			    КонецЕсли; 
			...
			//Партия_МССО_Объект.СтранаПроизводитель = Строка.HS_СтранаПроисхождения.Ссылка;
			//Партия_МССО_Объект.СтранаПроизводитель = Строка.Номенклатура.СтранаПроисхождения;
			Партия_МССО_Объект.СистемнаяСерияПартииМССО = Строка.Серия;
			
			Попытка
				Партия_МССО_Объект.Записать();
			Исключение
				//ОписаниеОшибки()
			КонецПопытки; 
			...
			СерияСистемная = Справочники.СерииНоменклатуры.НайтиПоКоду(Строка.Серия.Код).ПолучитьОбъект();
			СерияСистемная.Партия_МССО = Строка.Партия_МССО;
			ПартияСистемнаяИмя = СокрЛП(СерияСистемная.Наименование);
			Если ЗначениеЗаполнено(Строка.КодТНВЭД) Тогда		
				СерияСистемная.КодТНВЭД = Строка.КодТНВЭД;
			КонецЕсли; 
			//СерияСистемная.Наименование = ""+СокрЛП(Партия_МССО_Объект.Код)+" ("+Прав(ПартияСистемнаяИмя,8)+")"; 
			СерияСистемная.Наименование = ""+СокрЛП(Партия_МССО_Объект.Код); 
			Если СерияСистемная.ГоденДо = Дата(1,1,1) Тогда
				 СерияСистемная.ГоденДо = Строка.Партия_МССО.СрокДействияАнализа;
			КонецЕсли; 
			
			//Попытка
				СерияСистемная.Записать();
			Исключение
		...
		Иначе
			Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
				Если ЭтотОбъект.Склад = Справочники.Склады.Склад3 Тогда
					Сообщить("Строка № "+Строка.НомерСтроки + " не заполнена ПАРТИЯ МССО");
					Отказ = Истина;
				КонецЕсли; 
			КонецЕсли; 
		...
		Если ЗначениеЗаполнено(Строка.ПартияПоставщика) Тогда
			Попытка
				
			СерияСистемная = Справочники.СерииНоменклатуры.НайтиПоКоду(Строка.Серия.Код).ПолучитьОбъект();
			СерияСистемная.Наименование = СокрЛП(СерияСистемная.Наименование)+" / "+Строка.ПартияПоставщика;
			
			Если ЗначениеЗаполнено(Строка.СрокГодности) Тогда
			
				СерияСистемная.Наименование = СокрЛП(СерияСистемная.Наименование)+" / " + Формат(Строка.СрокГодности,"ДЛФ=D"); 
			КонецЕсли;
					
            СерияСистемная.СрокГодности = Строка.СрокГодности;
			СерияСистемная.ПартияПоставщика = Строка.ПартияПоставщика;
			СерияСистемная.НомерКачественногоУдостоверения = Строка.НомерКачественногоУдостоверения;
			//Попытка
				СерияСистемная.Записать();
			Исключение
			Сообщить(ОписаниеОшибки());	
	...
	Если ЭтотОбъект.РучнаяКорректировкаДатыПлатежа = Ложь Тогда
		Если ЭтотОбъект.Договор.ОтсрочкаОплатыПоДоговору < 9999 Тогда
			Если КонецДня(ЭтотОбъект.Дата) < КонецДня(Константы.СезоннаяСкидкеДействуетДо.Получить()) Тогда   //////77w qaws
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
    ...
    ЕстьНеЗаполненыйКодТНВЭД = Ложь;
    Для каждого Строка Из ЭтотОбъект.Товары Цикл
    	Если НЕ ЗначениеЗаполнено(Строка.Номенклатура.КодТНВЭД) Тогда
    	 	Сообщить("Строка № "+Строка.НомерСтроки+" у номенкатуры "+Строка.Номенклатура+" не заполнен код ТНВЭД");
    		ЕстьНеЗаполненыйКодТНВЭД = Истина;
		КонецЕсли; 
		
		Если ЭтотОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика 
			И ЭтотОбъект.Дата >= Дата("20221101") И  ОбщийМодуль77w.ПроверитьРегулированиеЦенГосударством_Оптовых(Строка.Номенклатура,ЭтотОбъект.Дата) Тогда
			Если Строка.ПерваяЦена = 0 Тогда
				Если РольДоступна("АдминистраторСистемы") Тогда
					Сообщить("В строке " + Строка.НомерСтроки + " не заполнена Цена изготовителя");				
				Иначе
	...
	Иначе	
		Если ЕстьНеЗаполненыйКодТНВЭД = Истина Тогда
		Сообщить("В документе  у номенклатуры есть не заполненные коды ТНВЭД. Проведение не возможно.");
		Отказ = Истина;
		КонецЕсли; 
	КонецЕсли; 
	
	
	Если ЭтотОбъект.Дата < Дата(2019,11,01,0,0,0) Тогда
		
		Если ЭтотОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика Тогда
			Если ЭтотОбъект.Договор = Справочники.ДоговорыКонтрагентов.ПустаяСсылка() Тогда
				Сообщить("Не заполнен договор. Документ не проводится");
				Отказ = Истина;
			КонецЕсли; 
		КонецЕсли; 
	Иначе
	...
	// HS
	Движения.ТоварыОрганизаций.Записать(); // без этого не распределяются доп. расходы из документа на самого себя
	ЗапасыСервер.ОтразитьТоварыОрганизацийКОформлению(ДополнительныеСвойства, Движения, Отказ);
	//jadmin Поправили расчет
	...
	Движения.ТоварыСОбазятельноСтикеровкой.Записывать = Истина; 
	ТЗ_Товары = ЭтотОбъект.Товары.Выгрузить();
	ТЗ_Товары.Свернуть("Номенклатура, Серия","Количество");

	Для Каждого ТекСтрокаТовары Из ТЗ_Товары Цикл  
		Если ТекСтрокаТовары.Номенклатура.ТребуетсяМаркировка = ИСТИНА Тогда  
			
				Движение = Движения.ТоварыСОбазятельноСтикеровкой.Добавить();
				Движение.ВидДвижения = ВидДвиженияНакопления.Приход;
				Движение.Период = Дата;
				Движение.Номенклатура = ТекСтрокаТовары.Номенклатура; 
				Движение.Серия = ТекСтрокаТовары.Серия; 
				Движение.КоличествоПриход = ТекСтрокаТовары.Количество; 	
	...
	Для каждого Строка Из ЭтотОбъект.Товары Цикл
		...
		///jadmin заполняем данные поставщика в серию
		//Если ОбщийМодуль77w.ПроверитьРегулированиеЦенГосударством_Розничных(Строка.Номенклатура,ЭтотОбъект.Дата) = ИСТИНА 
		//	ИЛИ ОбщийМодуль77w.ПроверитьРегулированиеЦенГосударством_Оптовых(Строка.Номенклатура,ЭтотОбъект.Дата) Тогда
		
		СерияСсылка = Строка.Серия.ПолучитьОбъект();
		
		Если НЕ ЗначениеЗаполнено(СерияСсылка.СтавкаНДС) Тогда				
			СерияСсылка.СтавкаНДС = Строка.Номенклатура.СтавкаНДС;				
		КонецЕсли;   
		
		Если НЕ ЗначениеЗаполнено(СерияСсылка.СтавкаНДСПоставщик) Тогда				
			СерияСсылка.СтавкаНДСПоставщик = Строка.СтавкаНДС;				
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(СерияСсылка.НаименованиеНоменклатурыПрихода) Тогда				
			СерияСсылка.НаименованиеНоменклатурыПрихода = Строка.НаименованиеНоменклатурыПоставщика;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(СерияСсылка.КодТНВЭД) Тогда				
			СерияСсылка.КодТНВЭД = Строка.Номенклатура.КодТНВЭД;
		КонецЕсли;			
		
		Если НЕ ЗначениеЗаполнено(СерияСсылка.ЕдиницаИзмеренияПрихода) Тогда				
			Если ЗначениеЗаполнено(Строка.Упаковка) Тогда
				СерияСсылка.ЕдиницаИзмеренияПрихода = Строка.Упаковка;
			Иначе
				СерияСсылка.ЕдиницаИзмеренияПрихода = Строка.Номенклатура.ЕдиницаИзмерения;
			КонецЕсли;
		КонецЕсли;	
		
		
		СерияСсылка.Записать();
			...
			Если Строка.Серия.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика Тогда
				...
				ЦенаПоставщика = 0;
				ЦенаПоставщика = строка.ПерваяЦена;
				
				СерияСсылка.ЦенаПоставщика = ЦенаПоставщика;
				СерияСсылка.ФСОФСН = ЭтотОбъект.ФСОФСН;
				Если ЦенаПоставщика <> 0 Тогда
					Если Строка.ЦенаСоСкидкой < Строка.ПерваяЦена Тогда 
						СерияСсылка.ВидСкидкиПоставщика = ЭтотОбъект.ВидСкидкиПоставщика;
						ПроцентСкидкиПоставщика = 0 ;
						ПроцентСкидкиПоставщика = Окр((Строка.ЦенаСоСкидкой/ЦенаПоставщика - 1) *100,2)*(-1);
						
						Если ПроцентСкидкиПоставщика <> Строка.ПроцентСкидкиСПервойЦены Тогда
							
							Сообщить("В строке "+Строка.НомерСтроки+" процент скидки поставщика (" +Строка.ПроцентСкидкиСПервойЦены+"%) не совпадает с расчетным значением (" +ПроцентСкидкиПоставщика + "%)");
							
						КонецЕсли;
						
						СерияСсылка.ПроцентСкидкиПоставщика = Строка.ПроцентСкидкиСПервойЦены;
						СерияСсылка.РазмерСкидкиПоставщика = ЦенаПоставщика - Строка.ЦенаСоСкидкой;
					ИначеЕсли Строка.ЦенаСоСкидкой > Строка.ПерваяЦена Тогда 
						Сообщить("ВНИМАНИЕ! Неприемлемая цена поступления для дальнейшей реализации!");
					КонецЕсли;
				КонецЕсли;	
		...
		//КонецЕсли;
		 ...
		 ////jadmin проверка необходимости возврата на склад (рег.цен)       Отключил 10-01-2023
		 ////////77w
		 //Если ПроверкаУстановкиЦенПоСериям_Розничные(Строка.Номенклатура) = ЛОЖЬ Тогда
		 //    Если ПроверитьОстатокПоНоменклатуреВРознице(Строка.Номенклатура,Строка.ЦенаСоСкидкой) = ИСТИНА Тогда
		 //   	 Если РольДоступна("АдминистраторСистемы") Тогда
		 //   		 Сообщить("По номенклатуре "+Строка.Номенклатура+" имеются остатки в рознице с себестоимостью больше прихода, НЕОБХОДИМО ВЕРНУТЬ ТОВАРЫ с магазинов НА СКЛАД");
		 //   	 Иначе
		 //   		 Сообщить("По номенклатуре "+Строка.Номенклатура+" имеются остатки в рознице с себестоимостью больше прихода, НЕОБХОДИМО ВЕРНУТЬ ТОВАРЫ с магазинов НА СКЛАД");
		 //   		 Отказ = ИСТИНА;
		 //   	 КонецЕсли;
		 ...
		 Если ПроверкаУстановкиЦенПоСериям_Опт(Строка.Номенклатура) = ЛОЖЬ Тогда	
			 Сообщить("По номенклатуре "+Строка.Номенклатура+" НЕ УСТАНОВЛЕНЫ ЦЕНЫ по сериям, остатки которых имеются на складе. НЕОБХОДИМО установить цены на серии номенклатуры(оптовые)");
			 //Отказ = ИСТИНА;	
		КонецЕсли;
		///
	...
	//	Если ЭтотОбъект.Валюта = справочники.Валюты.НайтиПоКоду("933") Тогда
	//		КурсВалют = 1;
	//	Иначе
	//		КурсВалют = ПолучитьКурсВалют(ЭтотОбъект.Дата,ЭтотОбъект.Валюта);
	//	КонецЕсли;
	//	
	//	Если КурсВалют <> Неопределено Тогда
	//		Для каждого стрТовар Из ЭтотОбъект.Товары Цикл
	//			СерияСсылка = стрТовар.Серия.ПолучитьОбъект();
	//			СерияСсылка.СебестоимостьНаДатуПоступления = стрТовар.Цена * КурсВалют;
	//			СерияСсылка.Записать();	
	//		КонецЦикла; 
	//	Иначе
	//		Сообщить("Не определен курс валют");
	//	КонецЕсли; 
	//
	
	//Себестоимость товара записываем в серию
	Движ = РегистрыНакопления.СебестоимостьТоваров.СоздатьНаборЗаписей();
	Движ.Отбор.Регистратор.Значение = ЭтотОбъект.Ссылка;
	Движ.Прочитать();

	Для Каждого Строка Из Движ Цикл	
		СерияСсылка  = Строка.АналитикаУчетаНоменклатуры.Серия.ПолучитьОбъект();
		СерияСсылка.СебестоимостьНаДатуПоступления = Окр(Строка.СтоимостьРегл / Строка.Количество,2) + СерияСсылка.ДополнительныеРасходы + СерияСсылка.ТаможенныеРасходы;
	    СерияСсылка.Записать();	
	КонецЦикла;
	////
	...
	Если ЭтотОбъект.НужнаЭСЧФ = Истина Тогда
...
Процедура ЗаполнитьДокументНаОснованииЗаказаПоставщику(Знач ДокументОснование, Знач СкладПоступления = Неопределено, РеквизитыЗаказа = Неопределено)
	...
	Для каждого Строка Из ЭтотОбъект.Товары Цикл
		...
		НайденныеСтроки = ДокументОснование.Товары.НайтиСтроки(Отбор);
		
		Для каждого НайдСтр Из НайденныеСтроки Цикл  
			Строка.НаименованиеНоменклатурыПоставщика = НайдСтр.НаименованиеНоменклатурыПоставщика;	
		КонецЦикла;
...
Функция  ПолучитьКурсВалют(Дата,Валюта)

	КурсВалют = Неопределено;
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		КурсВалют = ВыборкаДетальныеЗаписи.Курс/ВыборкаДетальныеЗаписи.Кратность;
	КонецЦикла;
...
Функция Удалить_ПроверитьОстатокПоНоменклатуреВРознице(Номенклатура,Цена)
	
	ЕстьОстаткиВРознице = ЛОЖЬ;
	Если ОбщийМодуль77w.ПроверитьРегулированиеЦенГосударством_Розничных(Номенклатура,ЭтотОбъект.Дата) = ИСТИНА Тогда
		...
		|	ТоварыНаСкладахОстатки.Серия.Цена КАК СерияЦена
		|ИЗ
		|	РегистрНакопления.ТоварыНаСкладах.Остатки(&Период, ) КАК ТоварыНаСкладахОстатки
		|ГДЕ
		|	ТоварыНаСкладахОстатки.Номенклатура = &Номенклатура
		|	И ТоварыНаСкладахОстатки.Склад.ТипСклада = ЗНАЧЕНИЕ(Перечисление.ТипыСкладов.РозничныйМагазин)
		|	И ТоварыНаСкладахОстатки.Серия.Цена > &Цена";
		...
		Если РезультатЗапроса.Количество()>0 Тогда
			ЕстьОстаткиВРознице = ИСТИНА;
		КонецЕсли;
		
		Возврат ЕстьОстаткиВРознице;
...
Функция ПроверкаУстановкиЦенПоСериям_Опт(Номенклатура)
	
	УстановленыЦеныПоСериям = ИСТИНА;
	
	Если ОбщийМодуль77w.ПроверитьРегулированиеЦенГосударством_Оптовых(Номенклатура,ЭтотОбъект.Дата) = ИСТИНА Тогда
		...
		|ГДЕ
		|	ТоварыНаСкладахОстатки.Номенклатура = &Номенклатура
		|	И ТоварыНаСкладахОстатки.Склад.ТипСклада = ЗНАЧЕНИЕ(Перечисление.ТипыСкладов.ОптовыйСклад)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ЦеныНоменклатурыПоСериям_МССОСрезПоследних.Номенклатура,
		|	ЦеныНоменклатурыПоСериям_МССОСрезПоследних.Серия,
		|	0,
		|	ЕСТЬNULL(ЦеныНоменклатурыПоСериям_МССОСрезПоследних.Цена, 0)
		|ИЗ
		|	РегистрСведений.ЦеныНоменклатурыПоСериям_МССО.СрезПоследних КАК ЦеныНоменклатурыПоСериям_МССОСрезПоследних
		|ГДЕ
		|	ЦеныНоменклатурыПоСериям_МССОСрезПоследних.ВидЦены = &ВидЦены
		|	И ЦеныНоменклатурыПоСериям_МССОСрезПоследних.Номенклатура = &Номенклатура
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	СУММА(ТАБИТОГ.ВНаличииОстаток) КАК ВНаличииОстаток,
		|	СУММА(ТАБИТОГ.Цена) КАК Цена
		|ИЗ
		|	ТАБИТОГ КАК ТАБИТОГ
		|
		|СГРУППИРОВАТЬ ПО
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Если ВыборкаДетальныеЗаписи.Цена = 0 Тогда		
				УстановленыЦеныПоСериям = ЛОЖЬ;			
			КонецЕсли;	
		КонецЦикла;
		
		Возврат УстановленыЦеныПоСериям;
