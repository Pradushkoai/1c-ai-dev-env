...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Объект.ВремяГотовности = Дата(1,1,1);
		Объект.Готов = Ложь;
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		|ГДЕ
		|	ПриходныйОрдерНаТовары.Распоряжение = &ВыбЗаказ
		...
		Если НашлиОрдер <> Неопределено Тогда
			Сообщить("Установлен запрет на редактирование заявки, т.к выписан Приходный ордер:"+Символы.ПС+НашлиОрдер);
			//ЭтаФорма.ТолькоПросмотр = Истина;
			//Элементы.Товары.Доступность = Ложь;
			//Элементы.ТоварыЗаполнитьСерииПо_FIFO_МССО.Доступность = Ложь;
			//Элементы.ТоварыПодборДляРозницы.Доступность = Ложь;
...
Процедура ПриЗакрытии()
    ...
    Если ЕстьПромыслы = Истина Тогда
    	Предупреждение("Необходимо прикрепить копию заключения об отнесении продукции к изделиям народных промыслов (ремесел)");
    КонецЕсли; 
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	//jadmin 77w Создание документа УстановкиЦен Закупочных
	Если Константы.НеСоздаватьУстановкуЦенПриПриходе.Получить() = Ложь Тогда
		...
		Запрос.УстановитьПараметр("Период", НачалоДня(ТекущийОбъект.Дата));
		...
		ВидЦены = Справочники.ВидыЦен.НайтиПоНаименованию("Закупочные",Истина);
		
		ДокУстЗакупЦен = Документы.УстановкаЦенНоменклатуры.ПустаяСсылка();
		
	Попытка
	
		Если ТекущийОбъект.ДокументУстановкиЗакупочныхЦен <> Документы.УстановкаЦенНоменклатуры.ПустаяСсылка() Тогда
			 ДокУстЗакупЦен = ТекущийОбъект.ДокументУстановкиЗакупочныхЦен.Ссылка.ПолучитьОбъект();
			 Попытка
			 	ДокУстЗакупЦен.Записать(РежимЗаписиДокумента.ОтменаПроведения);
			 Исключение
			     //ОписаниеОшибки()
			 КонецПопытки; 
			 //ДокУстЗакупЦен.Записать(РежимЗаписиДокумента.ОтменаПроведения);
		 Иначе
			 ...
			 ДокУстЗакупЦен =  Документы.УстановкаЦенНоменклатуры.СоздатьДокумент();
			 ДокУстЗакупЦен.Дата = ТекущийОбъект.Дата+1;
			 ДокУстЗакупЦен.Ответственный = ТекущийОбъект.Автор;
	     	 ДокУстЗакупЦен.Статус = Перечисления.СтатусыУстановокЦенНоменклатуры.Согласован;
			 
			 нов = ДокУстЗакупЦен.ВидыЦен.Добавить();
			 нов.ВидЦены = ВидЦены;
		     ДокУстЗакупЦен.Комментарий = "Установка закупочных цен по " + ТекущийОбъект.Ссылка;		 
		КонецЕсли; 
		
		ДокУстЗакупЦен.Товары.Очистить();
			
			
		ТЗ_ТЧ = ТекущийОбъект.Товары.Выгрузить();
		ТЗ_ТЧ.Свернуть("Номенклатура","КоличествоУпаковок,Сумма");
		
		Для каждого стр из ТЗ_ТЧ Цикл
			//Записываем только изменения цен
			ЦенаСтарая = ПолучитьТекущуюЦенуНоменклатуры(ВидЦены,стр.Номенклатура,ДокУстЗакупЦен.Дата);
			НоваяЦена = Окр((стр.Сумма/?(стр.КоличествоУпаковок>0,стр.КоличествоУпаковок,1))*Курс/Кратность,2);
			Если ЦенаСтарая = НоваяЦена Тогда			
				Продолжить;
			КонецЕсли; 
			 нов = ДокУстЗакупЦен.Товары.Добавить();		
			 нов.Номенклатура = стр.Номенклатура;
			 нов.Серия = Справочники.СерииНоменклатуры.ПустаяСсылка();
			 нов.ВидЦены = ВидЦены;
			 нов.Цена = НоваяЦена;
			 //нов.ЦенаИзмененаВручную = Истина;		 
		 КонецЦикла;		
		 
		 Если ДокУстЗакупЦен.Товары.Количество() > 0 Тогда
			 
			 ДокУстЗакупЦен.Записать(РежимЗаписиДокумента.Проведение);
			 ТекущийОбъект.ДокументУстановкиЗакупочныхЦен = ДокУстЗакупЦен.Ссылка; 
			 
		 КонецЕсли; 
				
	
	Исключение
	   Сообщить("Не удалось создать документ установки закупочных цен");
	КонецПопытки;
 
	 КонецЕсли; //Создание установки цен Закупочных	
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	HS_РаботаСДиалогамиКлиент.ТоварыПриНачалеРедактирования(ЭтаФорма, Элемент, НоваяСтрока, Копирование);			
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	//FF = Справочники.Номенклатура.НайтиПоНаименованию(ТекущаяСтрока.Номенклатура);
	
	//ТекущаяСтрока.ВидТМЦ = ТекущаяСтрока.Номенклатура.Cсылка;
	
	ТекущаяСтрока.КодТНВЭД = ПолучитьКодТНВЭД(ТекущаяСтрока.Номенклатура);       //jadmin
...
Процедура ТоварыКоличествоУпаковокПриИзменении(Элемент)
	...
	ТекущаяСтрока.ВесНеттоМССО = ТекущаяСтрока.КоличествоУпаковок;   ///////77w qaws
КонецПроцедуры
...
Процедура УстановитьУсловноеОформление()
	...
	Если Объект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика Тогда
		Элементы.ФСОФСН.Видимость = Ложь;
		Элементы.ВидСкидкиПоставщика.Видимость = Ложь;
		Элементы.ТоварыПерваяЦена.Видимость = Ложь;
		Элементы.ТоварыПроцентСкидкиСПервойЦены.Видимость = ЛОЖЬ;
	КонецЕсли; 
...
Процедура ПриИзмененииДоговораСервер()
	...
	Если ЗначениеЗаполнено(Объект.Договор) Тогда
		...
		Объект.ПорядокРасчетов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Договор,"ПорядокРасчетов");     //////77w qaws
	Иначе
		ЗаполнитьПорядокОплатыПоУмолчанию();
	КонецЕсли;

	Если ЗначениеЗаполнено(Объект.Договор) Тогда           //////77w qaws
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеПриИзменении(Элемент)
	...
	ИначеЕсли Элемент.Имя = "ТоварыКолГрузМест" Тогда
		ТоварыКолГрузМестПриИзменении(Элемент);
	ИначеЕсли Элемент.Имя = "ТоварыВесНетто" Тогда
		ТоварыВесНеттоПриИзменении(Элемент);
	ИначеЕсли Элемент.Имя = "ТоварыВес1Места" Тогда 
		ТоварыВес1МестаПриИзменении(Элемент);
	Иначе
...
Процедура УстановитьДоступностьЭлементовПоСтатусуСервер()
	...
	// HS добавил исключения	
	МассивЭлементов.Добавить("Товары;ПередНачаломДобавления,ПередУдалением,ПередНачаломИзменения;ТоварыHS_МестоПриобретения,ТоварыHS_СтранаПроисхождения,ТоварыHS_СтранаВвоза,ТоварыНаименованиеТовараВПартии,ТоварыВидТМЦ,ТоварыДатаИзготовления,ТоварыГоденДо,ТоварыНомер");
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		ТекущаяСтрока.ВидТМЦ = СтрокаТовара.Номенклатура.ВидТМЦ;
		ТекущаяСтрока.HS_МестоПриобретения = Перечисления.МестоПриобретения.РБ;
		ТекущаяСтрока.HS_СтранаВвоза = СтрокаТовара.Номенклатура.СтранаВвоза;
		ТекущаяСтрока.HS_СтранаПроисхождения = СтрокаТовара.Номенклатура.СтранаПроисхождения;
...
Процедура ПолучитьТоварыИзХранилища(АдресТоваровВХранилище)
	...
	Для Каждого СтрокаТоваров Из ТоварыИзХранилища Цикл
		...
		HS_ОбработкаТабличныхЧастей.РассчитатьЦенуСоСкидкой(СтрокаТЧТовары, Объект);
...
// HS - подключается программно
&НаКлиенте
Процедура HS_КолонкаТоварыПриИзменении(Элемент)
	Если HS_РаботаСДиалогамиКлиент.ПриИзмененииСтрокиТЧ(ЭтаФорма, "Товары", Элементы.Товары.ТекущаяСтрока, Элемент.Имя) Тогда
		HS_КолонкаТоварыПриИзмененииНаСервере(Элементы.Товары.ТекущаяСтрока, Элемент.Имя)
	Конецесли;
КонецПроцедуры

&НаСервере
Процедура HS_КолонкаТоварыПриИзмененииНаСервере(ИдентификаторСтроки, Колонка)
	HS_РаботаСДиалогами.ПриИзмененииСтрокиТЧ(ЭтаФорма, "Товары", ИдентификаторСтроки, Колонка)
КонецПроцедуры



&НаСервере
Функция НайтиЦеныНаСервере()
...
&НаСервере
Процедура ПечатьЦенникаНаСервере(ТабДок,ВыбЗначениеЦен)
	Перем КомпонентШК;
	...
	//Макет = ПолучитьМакетНаСервере("МакетЭтикетки");
	...
	Для каждого СтрокаТовары Из  Объект.Товары Цикл
	   ...
	   Запрос.УстановитьПараметр("Номенклатура", СтрокаТовары.Номенклатура); // если поле на форме так называется       
	   Запрос.УстановитьПараметр("ВидЦены", ВыбЗначениеЦен);
	   ...
	   ШКДляДжестори = "";
	   ШКДляДжестори = СтрокаТовары.Номенклатура.ШКДляДжестори.Код;
	   ...
	   Если ШтрихКод = Неопределено Тогда
		   Сообщить("Строка №; "+СтрокаТовары.НомерСтроки+"   Не заполнен штрих-код: "+СтрокаТовары.Номенклатура);
	   	   Продолжить;
	   КонецЕсли; 
	   ...
	   //Для I = 1 По КолЭкз Цикл
		   
		   //Макет = ПолучитьМакетНаСервере("МакетЦенника");
		   //ОбластьШапка  = Макет.ПолучитьОбласть("ОбластьШтрихкода");
		   ...
		   //ОбластьШапка = Макет.ПолучитьОбласть("ГорТело");
		   //ОбластьШапка1 = Макет.ПолучитьОбласть("ВертТело");
		   Фирма = ""+"ОАО ""МинскСортСемОвощ""";
		   ОбластьШапка.Параметры.Фирма = ""+Фирма;
		   
		   ОбластьШапка.Параметры.Номенклатура = СокрЛП(СтрокаТовары.Номенклатура.НаименованиеПолное);
		   Если ЦенаТовараВременная = Неопределено Тогда
			   ЦенаТовараРуб = 0;
			   ЦенаТовараКопейки = 0;
			   Сообщить("Не найдена цена "+СокрЛП(СтрокаТовары.Номенклатура.НаименованиеПолное));
			   Продолжить;
		   КонецЕсли; 
		   ...
		   Если ЦенаТовараРуб <> 0 Тогда
			   ЦенаТовара = ""+ЦенаТовараРуб+"р."+ЦенаТовараКопейки+"к.";
		   Иначе    
			   ЦенаТовара = ""+ЦенаТовараКопейки+"к.";
		   КонецЕсли; 
		   
		   //ЦенаТовара = ""+ЦенаТовараРуб+"р."+ЦенаТовараКопейки+"к.";
		   ОбластьШапка.Параметры.ЦенаТовара = ЦенаТовара;
		   
		   //ОбластьШапка.Параметры.Партия = ""+""+Объект.Серия.Партия_МССО;
		   //ОбластьШапка.Параметры.Партия = ""+"Партия "+СокрЛП(Объект.Серия.Наименование);
		   
		   //СрокГодности = Объект.Серия.СрокИспользованияДляФасованныхСемян;
		   //СрокГодности = Формат(СрокГодности,"ДЛФ=D");
		   //СрокГодности = Сред(СрокГодности,4);
		   //ОбластьШапка.Параметры.Срок = ""+"Срок исп-ния: "+СрокГодности;
		   
		   //ОбластьШапка.Параметры.КолВУП = ""+"Кол-во "+Объект.Номенклатура.ЕдиницаДляОтчетов.КолВУпак+" шт";
		   //ОбластьШапка.Параметры.КолВУП = ""+"Кол-во "+КолВУП+" шт";
		   ...
		   //ПараметрыШтрихкода.Вставить("Штрихкод", Объект.Серия.ШтрихКодСерии);
		   ...
		   КонецПопытки; 
		   //ОбластьШапка.Рисунки.ШтрихКод.Картинка = МенеджерОборудованияВызовСервера.ПолучитьКартинкуШтрихкода(ПараметрыШтрихкода);
		   //Секция.Рисунки.ШтрихКод1.Картинка = МенеджерОборудованияВызовСервера.ПолучитьКартинкуШтрихкода(ПараметрыШтрихкода);
		   
		   ОбластьШапка.Параметры.Партия = ""+"п."+СтрокаТовары.Серия.Партия_МССО;
		   ОбластьШапка.Параметры.Страна = ""+СокрЛП(СтрокаТовары.Серия.СтранаПроисхождения.Наименование);
		   ОбластьШапка.Параметры.НомерДок = ""+" "+СокрЛП(СтрЗаменить(Объект.Номер,"-",""));
		   ОбластьШапка.Параметры.ДатаДок = ""+СокрЛП(Формат(Объект.Дата,"ДЛФ=D"));
		   ОбластьШапка.Параметры.ЕдИзмТовара = ""+ЕдИзмТовара;
...
&НаКлиенте
Процедура ПечатьЦенника(Команда)
	
	СписокТиповЦен = НайтиЦеныНаСервере();
	
	//ВыбЗначение = Справочники.ВидыЦен.НайтиПоНаименованию("Розничные");
	...
	Если ВыбЭлемент <> Неопределено Тогда 
		ВыбЗначениеЦен = ВыбЭлемент.Значение;
		//Иначе
		...
		//Если ВвестиЧисло(
		//	КолЭкз,
		//	"Введите количество экземпляров для печати",
		...
		//	Тогда
			...
			ТабДок = Новый ТабличныйДокумент;
			ПечатьЦенникаНаСервере(ТабДок,ВыбЗначениеЦен);
			ТабДок.Показать("Печать ценников по цене "+ВыбЗначениеЦен);	
...
&НаСервере
Процедура ПоказатьШтрихКодПриИзмененииНаСервере()
	...
	Для каждого Строка  Из Объект.Товары  Цикл
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Строка.ШтрихКод = ВыборкаДетальныеЗаписи.Штрихкод;
		КонецЦикла;
...
&НаКлиенте
Процедура ПоказатьШтрихКодПриИзменении(Элемент)
	ПоказатьШтрихКодПриИзмененииНаСервере();
КонецПроцедуры


&НаКлиенте
Процедура ГотовПриИзменении(Элемент)
	Если НЕ ЗначениеЗаполнено(Объект.ВремяГотовности) Тогда
		Объект.ВремяГотовности = ТекущаяДата();
	КонецЕсли; 
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьСтроки_Страной(Команда)
	Для каждого СтрокаТаб Из Объект.Товары Цикл
		
		Если ЗначениеЗаполнено(Объект.HS_СтранаВвоза) Тогда
			СтрокаТаб.HS_СтранаВвоза = Объект.HS_СтранаВвоза;
		КонецЕсли; 
		
		Если ЗначениеЗаполнено(Объект.HS_СтранаПроисхождения) Тогда
			СтрокаТаб.HS_СтранаПроисхождения = Объект.HS_СтранаПроисхождения;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Объект.HS_МестоПриобретения) Тогда
			СтрокаТаб.HS_МестоПриобретения = Объект.HS_МестоПриобретения;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Объект.СрокГодности) Тогда
			СтрокаТаб.СрокГодности = Объект.СрокГодности;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Объект.ПартияПоставщика) Тогда
			СтрокаТаб.ПартияПоставщика = Объект.ПартияПоставщика;
		КонецЕсли;
...
&НаКлиенте
Процедура ТоварыКолГрузМестПриИзменении(Элемент)
	...
	Попытка
		ТекущаяСтрока.Вес1Места = ТекущаяСтрока.ВесНеттоМССО / ТекущаяСтрока.КолГрузМест;   ///////77w qaws
	Исключение
	    //ОписаниеОшибки()
	КонецПопытки;  
...
&НаКлиенте
Процедура ТоварыВесНеттоПриИзменении(Элемент)
	ТоварыКолГрузМестПриИзменении("ВесНеттоМССО");
	//ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
	//
	//Попытка
	//	ТекущаяСтрока.Вес1Места = ТекущаяСтрока.ВесНеттоМССО / ТекущаяСтрока.КолГрузМест;   ///////77w qaws
...
&НаКлиенте
Процедура ТоварыВес1МестаПриИзменении(Элемент)
	// Вставить содержимое обработчика.
КонецПроцедуры


&НаСервере
Процедура ПечатьИмпортНаСервере(ТабДок)
	...
	Макет = ПолучитьМакетНаСервере("Импорт");
	
	
	ОбластьШапка = Макет.ПолучитьОбласть("Шапка");
	ОбластьШапка.Параметры.Склад = ЭтотОбъект.Склад;
	НомерДок = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Объект.Номер);
	ОбластьШапка.Параметры.НомерДок = НомерДок;
	ОбластьШапка.Параметры.ДатаДок = Формат(Объект.Дата,"ДЛФ=D");
	ОбластьШапка.Параметры.Договор = "Договор № "+СокрЛП(Объект.Договор.Номер)+ " от "+Формат(Объект.Договор.Дата,"ДЛФ=D");
	
	Продавец = ""+Объект.Контрагент+", "+СокрЛП(Объект.Контрагент.КонтактнаяИнформация[0].Представление);
    ...
    Запрос.Текст = 
        "ВЫБРАТЬ ПЕРВЫЕ 1
        |	ПоступлениеУслугПрочихАктивовРасходы.Ссылка КАК Ссылка
        |ИЗ
        |	Документ.ПоступлениеУслугПрочихАктивов.Расходы КАК ПоступлениеУслугПрочихАктивовРасходы
        |ГДЕ
        |	ПоступлениеУслугПрочихАктивовРасходы.Ссылка.ДокументОснование = &ДокументОснование";
	...
	Если ЗначениеЗаполнено(Объект.НомерВходящегоДокумента) Тогда
		ИныеДокументы = ""+СокрЛП(Объект.НомерВходящегоДокумента)+" от "+Формат(Объект.ДатаВходящегоДокумента,"ДЛФ=D");
	КонецЕсли; 
	
	CMR = "";
	
	Если ЗначениеЗаполнено(Объект.НомерCMR) Тогда
		CMR = ", CMR от "+Формат(Объект.ДатаCMR,"ДЛФ=D")+" № "+СокрЛП(Объект.НомерCMR);
	КонецЕсли; 
	
	//Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
	//	Транспорт = ""+ВыборкаДетальныеЗаписи[0].Расходы[0].Содержание+" от "+ВыборкаДетальныеЗаписи.Ссылка.Контрагент+" на сумму "+Формат(ВыборкаДетальныеЗаписи.Ссылка.СуммаДокумента,"ЧДЦ=2"+" руб.");
	//КонецЦикла;	
	
	ОбластьШапка.Параметры.Транспорт = ""+ИныеДокументы+CMR+Символы.ПС+Транспорт;
	
	СтоимостьТоваровВвалютеДоговора_Цел = Цел(Объект.СуммаДокумента);
	Копейки = (Объект.СуммаДокумента - Цел(Объект.СуммаДокумента))*100;
	...
	ВВалюте = Объект.Валюта.Наименование;
	
	ОбластьШапка.Параметры.ВВалюте = ""+ВВалюте;
	ОбластьШапка.Параметры.Копейки = Копейки;
	
	ВидКоп = "";
	
	Если СокрЛП(Объект.Валюта.Код) = "643" Тогда
		ВидКоп = ""+"коп.";
	ИначеЕсли СокрЛП(Объект.Валюта.Код) = "840" Тогда
		ВидКоп = ""+"центы";
	ИначеЕсли СокрЛП(Объект.Валюта.Код) = "978" Тогда
		ВидКоп = ""+"евроценты";
	Иначе
		ВидКоп = ""+"коп.";
	КонецЕсли; 
	...
	ОбластьШапка.Параметры.ВидКоп = ""+ВидКоп;

	СтоимостьТоваровВвалютеДоговора = ЧислоПрописью(СтоимостьТоваровВвалютеДоговора_Цел,"НП=Ложь",",,,,,,,,0");
	
	ОбластьШапка.Параметры.СтоимостьТоваровВвалютеДоговора = СтоимостьТоваровВвалютеДоговора;
    ...
    ЗапросПлат.Текст = 
        ...
        |ГДЕ
        |	СписаниеБезналичныхДенежныхСредствРасшифровкаПлатежа.Заказ = &ДокументОснование";
 
    ЗапросПлат.УстановитьПараметр("ДокументОснование", Объект.Ссылка);
 
    РезультатЗапросаПлат = ЗапросПлат.Выполнить().Выгрузить();
	А = 1;
	
	ДатаПредоплаты = Дата(1,1,1);
	СуммаПредоплаты	= 0;
	
	Если РезультатЗапросаПлат.Количество() > 0 Тогда
		Для каждого СтрокаПредоплаты Из РезультатЗапросаПлат Цикл
		 ДатаПредоплаты  = СтрокаПредоплаты.Ссылка.Дата;
		 //СуммаПредоплаты = СтрокаПредоплаты.Ссылка.СуммаДокумента;
		 СуммаПредоплаты = СуммаПредоплаты + СтрокаПредоплаты.Сумма;
		КонецЦикла; 
	КонецЕсли; 	
	...
	Кратность = Объект.Кратность;
	КратностьТаб = "";
	КурсПересчетаТаб = "";	
	
	Если Кратность > 1 Тогда
		КратностьТаб = ""+"/100 "+"рос.руб.";
	Иначе
		КратностьТаб = ""+"";
	КонецЕсли; 
	...
	СтоимостьБелкиЦифрами = 0;
	КурсПересчетаТаблицы  = 0;
	...
	Для каждого СтрокаПредоплаты Из РезультатЗапросаПлат Цикл
			...
			Запрос.УстановитьПараметр("Валюта", Объект.Валюта);
			Запрос.УстановитьПараметр("Период", НачалоДня(СтрокаПредоплаты.Ссылка.Дата));
			...
			ДатаПредоплаты = СтрокаПредоплаты.Ссылка.Дата;
			СуммаПредоплаты1 = СтрокаПредоплаты.Сумма;
			
			Если Объект.СуммаДокумента > СуммаПредоплаты1 Тогда
				
				КурсПересчетаТаб = КурсПересчетаТаб+""+Формат(КурсПредоплаты,"ЧДЦ=4")+" бел.руб. "+КратностьТаб+", частичная предоплата на сумму "+Формат(СуммаПредоплаты1,"ЧДЦ=2")+" ("+Формат(ДатаПредоплаты,"ДЛФ=D")+")"+Символы.ПС;
				//+ ""+Формат(Курс,"ЧДЦ=4")+" бел.руб. "+КратностьТаб+",на дату оприходования в неоплаченной сумме "+Формат((Объект.СуммаДокумента - СуммаПредоплаты1),"ЧДЦ=2")+" "+Объект.Валюта;	
				//КурсПересчетаТаблицы = КурсПредоплаты;
				КурсПересчетаТаблицы = Объект.КурсСУчетомАвансов;
				КурсПредоплаты = КурсПересчетаТаблицы;
			ИначеЕсли Объект.СуммаДокумента = СуммаПредоплаты Тогда
				КурсПересчетаТаб = ""+Формат(КурсПредоплаты,"ЧДЦ=4")+" бел.руб. "+КратностьТаб+", на дату предоплаты"+" ("+Формат(ДатаПредоплаты,"ДЛФ=D")+")";	
				КурсПересчетаТаблицы = КурсПредоплаты;
			КонецЕсли; 
		
	КонецЦикла;	
	
			Если Объект.СуммаДокумента > СуммаПредоплаты Тогда
				КурсПересчетаТаб = КурсПересчетаТаб +
				 ""+Формат(Курс,"ЧДЦ=4")+" бел.руб. "+КратностьТаб+",на дату оприходования в неоплаченной сумме "+Формат((Объект.СуммаДокумента - СуммаПредоплаты),"ЧДЦ=2")+" "+Объект.Валюта;	
			КонецЕсли; 
		//СтоимостьБелкиЦифрами = Объект.СуммаДокумента*(КурсПредоплаты/Кратность);
	...
	Иначе
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Если ВыборкаДетальныеЗаписи.Кратность > 1 Тогда
				КратностьТаб = ""+"/100 "+"рос.руб.";
			Иначе
				КратностьТаб = ""+"";
			КонецЕсли; 
		...
		КурсПересчетаТаб = ""+Формат(Курс,"ЧДЦ=4")+" бел.руб. "+КратностьТаб+",на дату оприходования";	
		//СтоимостьБелкиЦифрами = Объект.СуммаДокумента*(Курс/Кратность);
		КурсПересчетаТаблицы = Курс;
	...
	КонецЕсли; 
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			СтоимостьБелкиЦифрами = ВыборкаДетальныеЗаписи.СтоимостьРегл;
		КонецЦикла;
	
	
	//КурсПересчетаТаб = ""+Курс+" бел.руб. "+КратностьТаб+",на дату оприходования";	
	
	ОбластьШапка.Параметры.КурсПересчетаТаб = КурсПересчетаТаб;
	
	СтоимостьБелки = РаботаСКурсамиВалют.СформироватьСуммуПрописью(СтоимостьБелкиЦифрами, Справочники.Валюты.НайтиПоНаименованию("BYN",Истина),, "РубКоп");
	
	ОбластьШапка.Параметры.СтоимостьБелки = СтоимостьБелки;
	
	
	ТабДок.Вывести(ОбластьШапка);
	
	ИтогКоличество = 0;
	ИтогСтоимостьБелки = 0;
	ИтогСтоимостьТоваровВвалютеДоговора = 0;
	
	Для каждого Строка Из Объект.Товары Цикл
		
		ОбластьСтрока = Макет.ПолучитьОбласть("Строка");
		//ОбластьСтрока.Параметры.Наименование = Строка.Номенклатура.НаименованиеПолное;
		
		
		Если ЗначениеЗаполнено(Строка.Серия.НаименованиеНоменклатурыПрихода) Тогда
			ОбластьСтрока.Параметры.Наименование = Строка.Серия.НаименованиеНоменклатурыПрихода;
		Иначе
			ОбластьСтрока.Параметры.Наименование = Строка.Номенклатура.НаименованиеПолное;
		КонецЕсли;
		
		
		ОбластьСтрока.Параметры.КодТовара = Строка.Номенклатура.Код;
		ОбластьСтрока.Параметры.ЕдИзм = Строка.Упаковка.Наименование;
		Если ОбластьСтрока.Параметры.ЕдИзм = "" Тогда
			ОбластьСтрока.Параметры.ЕдИзм = Строка.Номенклатура.ЕдиницаИзмерения;
		КонецЕсли; 
		ОбластьСтрока.Параметры.ЕдИзмКод = Строка.Номенклатура.ЕдиницаИзмерения.Код;
		...
		|ГДЕ
		|	СебестоимостьТоваров.Регистратор = &Регистратор
		|	И СебестоимостьТоваров.АналитикаУчетаНоменклатуры = &АналитикаУчетаНоменклатуры";
		
		Запрос.УстановитьПараметр("АналитикаУчетаНоменклатуры", Строка.АналитикаУчетаНоменклатуры);
		Запрос.УстановитьПараметр("Регистратор", Объект.Ссылка);
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			СтоимостьБелки = ВыборкаДетальныеЗаписи.СтоимостьРегл;
		КонецЦикла;
		
	
		
		
		ОбластьСтрока.Параметры.СтоимостьБелки = СтоимостьБелки;
		
		Цена = 0;
		Попытка
			Цена = Окр(СтоимостьБелки/Строка.Количество,2);
		Исключение
		    //ОписаниеОшибки()
		КонецПопытки; 
		ОбластьСтрока.Параметры.Цена = Цена;
		ОбластьСтрока.Параметры.СтоимостьТоваровВвалютеДоговора = Строка.Сумма;
		
		
		ИтогКоличество = ИтогКоличество + Строка.Количество;
		ИтогСтоимостьБелки = ИтогСтоимостьБелки + СтоимостьБелки;
		ИтогСтоимостьТоваровВвалютеДоговора = Объект.СуммаДокумента;
	...
	ОбластьИтого.Параметры.ИтогСтоимостьБелки = ИтогСтоимостьБелки;
  
  Объект.ОсобыеУсловияПеревозкиОписание = ИтогСтоимостьБелки;
  
  //
	//Если ИтогСтоимостьБелки <> СтоимостьБелкиЦифрами Тогда
	//	ОбластьИтого.Параметры.ИтогСтоимостьБелки = СтоимостьБелкиЦифрами;
	//КонецЕсли; 
	
	
	ОбластьИтого.Параметры.ИтогСтоимостьТоваровВвалютеДоговора = ИтогСтоимостьТоваровВвалютеДоговора;
	
	ТабДок.Вывести(ОбластьИтого);
	
	ОбластьПодвал = Макет.ПолучитьОбласть("Подвал");
    
    Если ЗначениеЗаполнено(Объект.ЗаказПоставщику) = Истина Тогда
        ЗаказОформил = "";
        ЗаказОформилДолжность = "";
        ...
        Запрос.УстановитьПараметр("ФизическоеЛицо", Объект.ЗаказПоставщику.Менеджер.ФизическоеЛицо);
        ...
        Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
            ЗаказОформил = ""+ВыборкаДетальныеЗаписи.Ссылка;
            ЗаказОформилДолжность = ""+ВыборкаДетальныеЗаписи.Должность;
        КонецЦикла;
        
        
        
        
        ОбластьПодвал.Параметры.ЗаказОформил = ЗаказОформил;
        ОбластьПодвал.Параметры.ЗаказОформилДолжность = ЗаказОформилДолжность;
    КонецЕсли; 
    
    
    
        РасчетПроизвел = "";
        РасчетПроизвелДолжность = "";
        ...
        Запрос.УстановитьПараметр("ФизическоеЛицо", Объект.Менеджер.ФизическоеЛицо);
        ...
        Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
            РасчетПроизвел = ""+ВыборкаДетальныеЗаписи.Ссылка;
            РасчетПроизвелДолжность = ""+ВыборкаДетальныеЗаписи.Должность;
        КонецЦикла;
        
        
        
        
        ОбластьПодвал.Параметры.РасчетПроизвел = РасчетПроизвел;
        ОбластьПодвал.Параметры.РасчетПроизвелДолжность = РасчетПроизвелДолжность;
...
&НаКлиенте
Процедура ПечатьИмпорт(Команда)
	ТабДок = Новый ТабличныйДокумент;
	ПечатьИмпортНаСервере(ТабДок);
	ТабДок.Показать("Импорт");
КонецПроцедуры



&НаСервере
Процедура ПечатьИнформацииОрассадеНаСервере(ТабДок)
	ТабДок = Новый ТабличныйДокумент;
	ТабДок.ОриентацияСтраницы=ОриентацияСтраницы.Портрет;
	...
	ЕстьВыделенныеСтроки = 0;
	Для Каждого Строка Из Элементы.Товары.ВыделенныеСтроки Цикл
		
		ЕстьВыделенныеСтроки = ЕстьВыделенныеСтроки + 1;
	КонецЦикла;
	...
	Для каждого СтрокаТовары Из  Объект.Товары Цикл
	   ...
	   //КонецЕсли; 
		   ...
		   Макет = ПолучитьМакетНаСервере("ИнформацияОрассаде");
		   ...
		   ОбластьШапка.Параметры.НоменклатураТаб = ""+СокрЛП(СтрокаТовары.Номенклатура.Наименование)+Символы.ПС+""+ШтрихКод;
  	...
  	КонецЦикла; 
		//ТабДок.Область("C4").ШиринаКолонки = 0;
		//ТабДок.Область("C4").Видимость = Ложь;
		
	    ТабДок.Область("C5").ШиринаКолонки = 0;
		ТабДок.Область("C5").Видимость = Ложь;
		//ТабДок.УдалитьОбласть("C1:C1");
КонецПроцедуры
...
&НаКлиенте
Процедура ПечатьИнформацииОрассаде(Команда)
	
	ТабДок = Новый ТабличныйДокумент;
		ПечатьИнформацииОрассадеНаСервере(ТабДок);
	ТабДок.Показать("Печать информации о рассаде и саженцах");	
	
КонецПроцедуры



//jadmin для установки цен по приходу
Функция ПолучитьТекущуюЦенуНоменклатуры(ВидЦены,Номенклатура,Дата)
...
Функция ПолучитьКодТНВЭД(Номенклатура)
	
	Возврат Номенклатура.КодТНВЭД;	
	
КонецФункции // ()

&НаСервере
Функция ПроверитьЧтоСписываем()
	ЧтоСписываем = Неопределено;
	Для каждого стр Из Объект.Товары Цикл
		Если стр.Номенклатура.Родитель = Справочники.Номенклатура.НайтиПоКоду("00-00000020") // Картофель
			ИЛИ стр.Номенклатура.Родитель.Родитель = Справочники.Номенклатура.НайтиПоКоду("00-00000020") // Картофель
			ИЛИ стр.Номенклатура.Родитель.Родитель.Родитель = Справочники.Номенклатура.НайтиПоКоду("00-00000020") // Картофель 
			Тогда
			  ЧтоСписываем = Справочники.Номенклатура.НайтиПоКоду("00-00000020");
		  ИначеЕсли стр.Номенклатура.Родитель = Справочники.Номенклатура.НайтиПоКоду("00-00000019") // Чеснок
			ИЛИ стр.Номенклатура.Родитель.Родитель = Справочники.Номенклатура.НайтиПоКоду("00-00000019") // Чеснок
			ИЛИ стр.Номенклатура.Родитель.Родитель.Родитель = Справочники.Номенклатура.НайтиПоКоду("00-00000019") // Чеснок 
			Тогда
			  ЧтоСписываем = Справочники.Номенклатура.НайтиПоКоду("00-00000019");
		КонецЕсли;
		
	
	КонецЦикла; 

	Возврат ЧтоСписываем;
КонецФункции // ()
 


&НаСервере
Функция СписатьКартофельПоНормамНаСервере(ДанныеФормы)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
		НашлиОрдер = Неопределено;
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Если (ТипЗнч(ВыборкаДетальныеЗаписи.Ссылка) = Тип("ДокументСсылка.ПриходныйОрдерНаТовары"))	Тогда 
				Если ВыборкаДетальныеЗаписи.Ссылка.Проведен И ВыборкаДетальныеЗаписи.Ссылка.Статус = Перечисления.СтатусыПриходныхОрдеров.Принят Тогда
					НашлиОрдер = ВыборкаДетальныеЗаписи.Ссылка;
				КонецЕсли; 
			КонецЕсли;	
		КонецЦикла;	 
		
		Если Объект.ДлинаМаршрута = 0 Тогда
			Сообщить("Не заполнена длина маршрута в документе "+Объект.Ссылка);
			Возврат ДанныеФормы;
		КонецЕсли; 
		
		Если НашлиОрдер = Неопределено Тогда
			Сообщить("Еще нет ордера на оприходование");
			Возврат ДанныеФормы;
		КонецЕсли; 
	...
	ДанныеФормы.Дата = Объект.Дата+10;
	ДанныеФормы.Склад =  Объект.Склад;
	ДанныеФормы.СтатьяРасходов = ПланыВидовХарактеристик.СтатьиРасходов.НайтиПоКоду("00-000238");	
	ДанныеФормы.Основание = Объект.Ссылка;
	ДанныеФормы.ИсточникИнформацииОЦенахДляПечати  = Перечисления.ИсточникиИнформацииОЦенахДляПечати.ПоСебестоимости;
	ДанныеФормы.Комментарий = "Основание: "+Объект.Ссылка+" списание по нормам";	
	
	ДлинаМаршрута = 0;
	ДлинаМаршрута = Объект.ДлинаМаршрута;
	
	ЭтоЗима   = Неопределено;
	ЭтоВесна  = Неопределено;
	ЭтоЛето   = Неопределено;
	ЭтоОсень  = Неопределено;
	
	Если Месяц(Объект.Дата) = 12 ИЛИ Месяц(Объект.Дата) = 1 ИЛИ Месяц(Объект.Дата) = 2 Тогда
		ЭтоЗима = Истина;
	ИначеЕсли Месяц(Объект.Дата) = 3 ИЛИ Месяц(Объект.Дата) = 4 ИЛИ Месяц(Объект.Дата) = 5 Тогда
		ЭтоВесна = Истина;
	ИначеЕсли Месяц(Объект.Дата) = 6 ИЛИ Месяц(Объект.Дата) = 7 ИЛИ Месяц(Объект.Дата) = 8 Тогда
		ЭтоЛето = Истина;
	ИначеЕсли Месяц(Объект.Дата) = 9 ИЛИ Месяц(Объект.Дата) = 10 ИЛИ Месяц(Объект.Дата) = 11 Тогда
		ЭтоОсень = Истина;
	КонецЕсли; 
	
	Коэфф = 0;
	
	Если ЭтоЗима = ИСТИНА Тогда
		
		Если ДлинаМаршрута >= 10 И ДлинаМаршрута <= 25  Тогда
			Коэфф = 0.1;
		ИначеЕсли ДлинаМаршрута >= 26 И ДлинаМаршрута <= 50  Тогда
			Коэфф = 0.2;
		ИначеЕсли ДлинаМаршрута >= 51 И ДлинаМаршрута <= 75  Тогда
			Коэфф = 0.3;
		ИначеЕсли ДлинаМаршрута >= 76 И ДлинаМаршрута <= 1000000  Тогда
			Коэфф = 0.3;
		КонецЕсли; 
		
	ИначеЕсли ЭтоВесна = ИСТИНА Тогда
		...
		ИначеЕсли ДлинаМаршрута >= 51 И ДлинаМаршрута <= 75  Тогда
			Коэфф = 0.3;
		ИначеЕсли ДлинаМаршрута >= 76 И ДлинаМаршрута <= 10000000  Тогда
			Коэфф = 0.4;
		КонецЕсли; 
	...
	|	ПриходныйОрдерНаТоварыТовары.ВесБрутто КАК ВесБрутто,
	...
	|	ПриходныйОрдерНаТоварыТовары.КолВоГрузовыхМест КАК КолВоГрузовыхМест,
	...
	Запрос.УстановитьПараметр("Ссылка", НашлиОрдер);
	...
	Для каждого Строка Из РезультатЗапроса Цикл
		...
		ВесЕдТары = Строка.Номенклатура.ВесТары;
		ВсегоВесТары = Окр(ВесЕдТары * Строка.КолВоГрузовыхМест,3);
		
		ВесБруттоБезТары = Строка.ВесБрутто - ВсегоВесТары;
		
		КоличествоПоПриходномуДокументу = 0;
		
		КоличествоПоПриходномуДокументу = Строка.Количество;
		
		ПотериПослеВзвешивания = 0;
		ПотериПослеВзвешивания = КоличествоПоПриходномуДокументу - ВесБруттоБезТары; 
		
		СписываемПоНормам = 0;
		
		СписываемПоНормам = КоличествоПоПриходномуДокументу * Коэфф / 100;
		
		Если СписываемПоНормам >= ПотериПослеВзвешивания Тогда
			
			нстр = ДанныеФормы.Товары.Добавить();
			
			ЗаполнитьЗначенияСвойств(нстр,Строка);
			нстр.КолВоГрузовыхМест = 0;
			
			нстр.СтатусУказанияСерий = 14;
			нстр.Серия = Строка.Серия;
			нстр.Упаковка = Строка.Упаковка;
			нстр.Количество  = ПотериПослеВзвешивания;
			нстр.КоличествоУпаковок = ПотериПослеВзвешивания;
			
			нстр.ПричинаСписания = "Списание по нормам";
		Иначе
			нстр = ДанныеФормы.Товары.Добавить();
			...
			нстр.Серия = Строка.Серия;
			нстр.Упаковка = Строка.Упаковка;
			нстр.Количество  = СписываемПоНормам;
			нстр.КоличествоУпаковок = СписываемПоНормам;
...
&НаСервере
Функция СписатьЧеснокПоНормамНаСервере(ДанныеФормы)
	...
	ДанныеФормы.Дата = Объект.Дата+10;
	ДанныеФормы.Склад =  Объект.Склад;
	ДанныеФормы.СтатьяРасходов = ПланыВидовХарактеристик.СтатьиРасходов.НайтиПоКоду("00-000244");	//Чеснок
	ДанныеФормы.Основание = Объект.Ссылка;
	ДанныеФормы.ИсточникИнформацииОЦенахДляПечати  = Перечисления.ИсточникиИнформацииОЦенахДляПечати.ПоСебестоимости;
	ДанныеФормы.Комментарий = "Основание: "+Объект.Ссылка+" списание по нормам";	
	...
	Если ЭтоЗима = ИСТИНА Тогда
		...
		Если ДлинаМаршрута >= 10 И ДлинаМаршрута < 26  Тогда
			Коэфф = 0.1;
		ИначеЕсли ДлинаМаршрута >= 26 И ДлинаМаршрута < 51  Тогда
			Коэфф = 0.2;
		ИначеЕсли ДлинаМаршрута >= 51 И ДлинаМаршрута < 76  Тогда
			Коэфф = 0.3;
		ИначеЕсли ДлинаМаршрута >= 76 И ДлинаМаршрута < 101  Тогда
			Коэфф = 0.4;
		ИначеЕсли ДлинаМаршрута > 101 Тогда
			Коэфф = (Цел(ДлинаМаршрута / 100) - 0.1) + 0.4;	
		КонецЕсли; 
	...
	ИначеЕсли ЭтоЛето = ИСТИНА Тогда
		
		Если ДлинаМаршрута >= 1 И ДлинаМаршрута < 10  Тогда
		    Коэфф = 0.1;
		ИначеЕсли ДлинаМаршрута >= 10 И ДлинаМаршрута < 26  Тогда
			Коэфф = 0.2;
		ИначеЕсли ДлинаМаршрута >= 26 И ДлинаМаршрута < 51  Тогда
			Коэфф = 0.4;
		ИначеЕсли ДлинаМаршрута >= 51 И ДлинаМаршрута < 76  Тогда
	...
	ИначеЕсли ЭтоОсень = ИСТИНА Тогда
	...
	Для каждого Строка Из РезультатЗапроса Цикл
		...
		ВесЕдТары = Строка.Номенклатура.ВесТары;
		ВсегоВесТары = Окр(ВесЕдТары * Строка.КолВоГрузовыхМест,2);
...
Функция ЭтоКартофель()

	Возврат Справочники.Номенклатура.НайтиПоКоду("00-00000020"); //Картофель;

КонецФункции // ()

Функция ЭтоЧеснок()

	Возврат Справочники.Номенклатура.НайтиПоКоду("00-00000019"); // Чеснок;

КонецФункции // ()
 

&НаКлиенте
Процедура СписатьКартофельПоНормам(Команда)
	Форма = ПолучитьФорму("Документ.СписаниеНедостачТоваров.ФормаОбъекта"); 
	ДанныеФормы = Форма.Объект;
	ЧтоСписываем = ПроверитьЧтоСписываем();
	Если ЧтоСписываем = ЭтоКартофель()
		Тогда	
		ДанныеФормы = СписатьКартофельПоНормамНаСервере(ДанныеФормы);
	ИначеЕсли  ЧтоСписываем = ЭтоЧеснок() 
		Тогда
		ДанныеФормы = СписатьЧеснокПоНормамНаСервере(ДанныеФормы);
	Иначе
		Сообщить("Нормы списания не установлены");
	КонецЕсли;
	...
	Если ДанныеФормы.Товары.Количество() > 0  Тогда
		Форма.Открыть();
	Иначе
		Сообщить("Нет данных для списания");
	КонецЕсли; 

КонецПроцедуры

&НаКлиенте
Процедура ТоварыСуммаСкидкиСПервойЦеныОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	A=1;
	  //jadmin 
	 ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
	 СуммаСкидкиСПервойЦены = Число(Текст);
	Если ЗначениеЗаполнено(ТекущаяСтрока.ПерваяЦена) Тогда
			ТекущаяСтрока.ПроцентСкидкиСПервойЦены = СуммаСкидкиСПервойЦены / ТекущаяСтрока.ПерваяЦена * 100 ;
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ТоварыПроцентСкидкиСПервойЦеныОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
	ПроцентСкидки = Число(Текст);
	Если ЗначениеЗаполнено(ТекущаяСтрока.ПерваяЦена) Тогда
			ТекущаяСтрока.СуммаСкидкиСПервойЦены = ПроцентСкидки * ТекущаяСтрока.ПерваяЦена / 100 ;
	КонецЕсли;

КонецПроцедуры

&НаСервере
Процедура ЗаполнитьЦенуИмпИзгот_Ценой_НаСервере()
	 Для каждого СтрокаТЧ  Из Объект.Товары Цикл
	     СтрокаТЧ.ПерваяЦена = СтрокаТЧ.Цена;
	 КонецЦикла; 
КонецПроцедуры
 

&НаКлиенте
Процедура ЗаполнитьЦенуИмпИзгот_Ценой(Команда)
	//ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
	//ТекущаяСтрока.ПерваяЦена = ТекущаяСтрока.Цена;
	
	ЗаполнитьЦенуИмпИзгот_Ценой_НаСервере();
КонецПроцедуры
