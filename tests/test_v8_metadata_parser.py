...
def sample_cf_structure(tmp_path):
    ...
    (bsl_dir / "text").write_text(
        "// Тестовый модуль\nФункция Тест() Экспорт\n    Возврат 1;\nКонецФункции", encoding="utf-8-sig"
    )
...
def test_parse_handles_invalid_metadata(tmp_path):
    ...
    (tmp_path / "1" / "bad-uuid-1234").write_text("not a valid metadata", encoding="utf-8")
...
def test_skips_objects_without_name(tmp_path):
    ...
    (tmp_path / "1" / uuid).write_text("{1,\n{12,uuid1,uuid2,\n{0,\n{3,\n{0,0}}}}}", encoding="utf-8-sig")
