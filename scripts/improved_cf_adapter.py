...
from cf_extractor import V8Container, V8_SIGNATURE, V8_SIGNATURE_64
from v8_metadata_parser import TYPE_MAP_V2, TYPE_MAP_V1
...
def generate_object_xml(obj_type: str, uuid: str, name: str, synonym: str, 
                        comment: str = '', extra_props: str = '') -> str:
...
def generate_config_dump_info(objects: list) -> str:
    ...
    for obj_type, obj_name, obj_uuid in objects:
        if not obj_name:
            continue
        lines.append(f'\t\t<Metadata name="{obj_type}.{obj_name}" id="{obj_uuid}" configVersion="0000000000000000000000000000000000000000"/>')
...
OBJECT_TYPES_WITH_MODULES = {
    ...
    'InformationRegister', 'AccumulationRegister', 'ChartsOfAccounts',
    'ChartOfAccounts', 'ChartsOfCharacteristicTypes', 'ChartOfCharacteristicTypes',
    'BusinessProcess', 'Task', 'ExchangePlan', 'Enum',
    'CalculationRegister', 'AccountingRegister', 'ChartOfCalculationTypes',
    'FilterCriterion', 'Constant',
}


def convert_cf_to_xml_format(extracted_dir: Path, output_dir: Path,
...
def _read_bsl_module(objects_dir: Path, uuid: str, suffix: str) -> str:
    ...
    if (stripped.startswith('#Если') or 
        stripped.startswith('#Область') or
        stripped.startswith('Процедура') or
        stripped.startswith('Функция') or
        stripped.startswith('Перем') or
        stripped.startswith('//') or
        '&НаСервере' in stripped[:200] or
        '&НаКлиенте' in stripped[:200] or
        '#НаКлиентеНаСервере' in stripped[:200] or
        '#НаСервере' in stripped[:200]):
        return text
...
def _extract_nested_forms(objects_dir: Path, output_dir: Path, 
                          all_files: dict, stats: dict) -> int:
    ...
    for uuid, meta_path in all_files.items():
        ...
        parent_match = re.search(r'\{1,\s*\{0,\s*\{(\d+),\s*\{1,\s*\{0,\s*0,\s*([0-9a-f-]{36})\}', content, re.IGNORECASE)
        if not parent_match:
            continue
