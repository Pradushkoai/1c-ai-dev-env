...
def parse_comment_block(comment_block):
    ...
    for line in lines:
        ...
        if in_params:
            ...
            param_match = re.match(r"^([А-Яа-яA-Za-z_][А-Яа-яA-Za-z0-9_]*)\s*[-–—]\s*(.+)", line)
            if param_match:
...
def main():
    ...
    module_dirs = sorted([d for d in os.listdir(CONFIG_DIR) if os.path.isdir(os.path.join(CONFIG_DIR, d))])
...
def generate_markdown(modules, total_methods):
                ...
                for method in m["methods"]:
                    ...
                    if method["params"]:
                        ...
                        for p in method["params"]:
                            ...
                            default = (
                                f", по умолчанию: `{p.get('default_value', '')}`" if p.get("default_value") else ""
                            )
    ...
    lines.append(
        "4. **Глобальные** модули доступны без указания имени (например, `Сообщить()` вместо `ОбщегоНазначения.Сообщить()`)"
    )
    lines.append("5. **Привилегированные** игнорируют RLS — осторожно с правами доступа")
    lines.append(
        "6. **ServerCall** (`ВызовСервера`) — серверные методы, которые можно вызывать с клиента через `ОбщийМодуль.ИмяМетода()`"
    )
