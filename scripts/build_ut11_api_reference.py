#!/usr/bin/env python3
"""
Парсер API общих модулей 1С:Управление торговлей 11.
...
Создаёт indexes/ut11-api-reference.md — справочник API УТ11.
"""
...
CONFIG_DIR = PATHS.config_ut11 + '/CommonModules'
OUTPUT_INDEX = PATHS.ut11_api_reference_md
OUTPUT_JSON = PATHS.ut11_api_reference_json
...
def main():
    print(f'Парсинг общих модулей УТ11 из {CONFIG_DIR}...')
...
def generate_markdown(modules, total_methods):
    ...
    lines = []
    lines.append('# Справочник API: 1С:Управление торговлей 11')
    lines.append('')
    lines.append(f'> Сгенерировано: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}')
    lines.append(f'> Источник: общие модули УТ 11.3.4.197 (отраслевая)')
    ...
    lines.append('grep -B 2 -A 10 "Цена" /home/z/my-project/indexes/ut11-api-reference.md')
    lines.append('')
    lines.append('# Найти метод по имени модуля')
    lines.append('grep -A 5 "СкладыСервер" /home/z/my-project/indexes/ut11-api-reference.md')
    lines.append('```')
    lines.append('')
    lines.append('### Программный поиск через JSON')
    lines.append('```python')
    lines.append('import json')
    lines.append('with open("/home/z/my-project/indexes/ut11-api-reference.json") as f:')
