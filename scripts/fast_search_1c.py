...
try:
    from src.services.search import build_index as _build_tfidf, search as _search_tfidf, tokenize as _tokenize
...
def build_index(algo: str = 'bm25') -> None:
    ...
    if algo != 'tfidf':
        print(f'   Версия индекса: v2 (BM25 + триграммы + стеммер)')
        print(f'   Гибридный поиск: BM25 (0.75) + триграммы (0.25)')
    else:
        print(f'   Версия индекса: v1 (TF-IDF, legacy)')
...
def main() -> None:
    ...
    p_build.add_argument('--algo', choices=['bm25', 'tfidf'], default='bm25',
                         help='Алгоритм индексации (по умолчанию bm25)')
    ...
    p_search.add_argument('--algo', choices=['auto', 'bm25', 'tfidf'], default='auto',
