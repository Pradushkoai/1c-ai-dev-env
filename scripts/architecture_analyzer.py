...
class ArchitectureAnalyzer:
    ...
    def _collect_modules(self, config_dir: Path) -> list[ModuleInfo]:
        ...
        for obj_type in ['Catalogs', 'Documents', 'DataProcessors', 'Reports',
            ...
            for obj_dir in type_dir.iterdir():
                ...
                if mgr_bsl.exists():
                    modules.append(self._parse_module(mgr_bsl, obj_name + '.Менеджер', 'ManagerModule', obj_type, obj_name))
                # Form modules
    ...
    def _parse_module(self, bsl_file: Path, name: str, module_type: str,
                      parent_type: str = '', parent_name: str = '') -> ModuleInfo:
        """Парсинг BSL модуля."""
        try:
            content = bsl_file.read_text(encoding='utf-8-sig', errors='replace')
        except Exception:
            return ModuleInfo(name=name, file_path=str(bsl_file), module_type=module_type,
        ...
        for line in lines:
            ...
            for match in dep_pattern.finditer(stripped):
                ...
                if dep not in ('Элементы', 'ЭтаФорма', 'Объект', 'РеквизитФормыВЗначение',
                               'ЗначениеВРеквизитФормы', 'Стр', 'СтрНайти', 'СтрДлина',
                               'СтрЗаменить', 'СтрРазделить', 'СтрСоединить',
                               'Массив', 'Структура', 'Соответствие', 'СписокЗначений',
                               'ТаблицаЗначений', 'ДеревоЗначений', 'Запрос',
                               'Результат', 'Выборка', 'Справочники', 'Документы',
                               'РегистрыСведений', 'РегистрыНакопления', 'Константы',
                               'Перечисления', 'ПланыВидовХарактеристик', 'ПланыСчетов',
                               'БизнесПроцессы', 'Задачи', 'ПланыОбмена'):
