...
def get_actual_tools_count() -> int:
    ...
    with patch('src.project.Project'):
        ...
        handler = next(
            (h for req_type, h in server.request_handlers.items()
             if req_type == ListToolsRequest),
            None,
        )
        if handler is None:
            return 0
...
def main() -> int:
    ...
    if manifest_count != actual_count:
        errors.append(
            f'manifest.json: mcp_tools_count={manifest_count}, '
            f'но фактическое кол-во={actual_count}'
        )
    ...
    if badge_match:
        ...
        if badge_count != actual_count:
            errors.append(
                f'README.md: badge MCP tools={badge_count}, '
                f'но фактическое кол-во={actual_count}'
            )
    ...
    for m in inline_matches:
        if int(m) != actual_count:
            errors.append(
                f'README.md: inline "{m} MCP tools", '
                f'но фактическое кол-во={actual_count}'
            )
    ...
    if mcp_doc_path.exists():
        ...
        for line_num, line in enumerate(mcp_doc.split('\n'), 1):
            ...
            for m in matches:
                if int(m) != actual_count:
                    errors.append(
                        f'docs/MCP_INTEGRATION.md:{line_num}: "{m} tools", '
                        f'но фактическое кол-во={actual_count}'
                    )
    ...
    if arch_doc_path.exists():
        ...
        for m in matches:
            if int(m) != actual_count:
                errors.append(
                    f'docs/ARCHITECTURE.md: "MCP-сервер ({m} tools)", '
                    f'но фактическое кол-во={actual_count}'
                )
