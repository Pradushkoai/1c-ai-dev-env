...
def extract_method_info(html_content, file_name):
    ...
    # Синтаксис
    m = re.search(r'<p class="V8SH_chapter">Синтаксис:</p>(.+?)(?:<p class="V8SH_chapter">|<p class="V8SH_versionInfo">|<HR>)', html_content, re.DOTALL)
    if m:
...
def is_method_file(file_path):
    ...
    if name in ('0.html', '1.html') or re.match(r'^\d+\.html$', name):
        return False
    return True
...
def main():
    ...
    lines.append(f'> Источник: распакованные .hbk файлы из `syntax-helper/`')
    ...
    lines.append('  "title": "СправочникМенеджер.<Имя справочника>.НайтиПоКоду (CatalogManager.<Catalog name>.FindByCode)",')
    ...
    lines.append(f'| `indexes/syntax-helper-index.md` | этот файл | структурированный индекс |')
    lines.append(f'| `indexes/syntax-helper-index.json` | JSON | машиночитаемый индекс для программного поиска |')
    lines.append('')
    lines.append('---')
    lines.append('')
    ...
    lines.append('')
    lines.append('✅ **Глобальный контекст** — все функции доступные в любом модуле (СтрНайти, Сообщить, ТекущаяДата, и т.д.)')
    lines.append('✅ **Методы объектов метаданных** — справочников, документов, регистров, и т.д. (НайтиПоКоду, Записать, Проведён, и т.д.)')
