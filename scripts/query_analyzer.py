...
class QueryAnalyzer:
    ...
    def _check_no_params(self, query: str, line: int, lines: list[str]) -> list[QueryIssue]:
        ...
        if not re.search(r'&[А-Яа-я]', query):
            ...
            if re.search(r'ГДЕ\s+.*=\s*[0-9]', query, re.IGNORECASE) or \
               re.search(r'ГДЕ\s+.*=\s*"[^"]*"', query, re.IGNORECASE):
                issues.append(QueryIssue(
                    rule_id='Q003',
                    severity='LOW',
                    line=line,
    ...
    def _check_like_start_percent(self, query: str, line: int) -> list[QueryIssue]:
        ...
        if re.search(r'\bПОДОБНО\s+"?%', query, re.IGNORECASE) or \
           re.search(r'\bLIKE\s+"?%', query, re.IGNORECASE):
            issues.append(QueryIssue(
                rule_id='Q005',
                severity='HIGH',
                line=line,
    ...
    def _check_no_distinct(self, query: str, line: int) -> list[QueryIssue]:
        ...
        if re.search(r'\bСОЕДИНЕНИЕ\b', query, re.IGNORECASE) or \
           re.search(r'\bJOIN\b', query, re.IGNORECASE):
            if not re.search(r'\bРАЗЛИЧНЫЕ\b', query, re.IGNORECASE) and \
               not re.search(r'\bDISTINCT\b', query, re.IGNORECASE):
    ...
    def get_stats(self, issues: list[QueryIssue]) -> dict:
