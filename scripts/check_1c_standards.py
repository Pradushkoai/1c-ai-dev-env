...
GLOBAL_CONTEXT_NAMES = {
    'Документы', 'Справочники', 'Регистры', 'Метаданные', 'Константы',
    'Пользователи', 'Сеансы', 'ПараметрыСеанса', 'Обработки', 'Отчеты',
    'Перечисления', 'ПланыСчетов', 'ПланыОбмена', 'ПланыВидовРасчета',
    'ПланыВидовХарактеристик', 'БизнесПроцессы', 'Задачи',
}
GLOBAL_CONTEXT_VAR_PATTERN = re.compile(
    r'\bПерем\s+(' + '|'.join(GLOBAL_CONTEXT_NAMES) + r')\b', re.IGNORECASE
)
...
SUBQUERY_IN_SELECT_PATTERN = re.compile(
    r'ВЫБРАТЬ\s+.*?\(ВЫБРАТЬ', re.IGNORECASE | re.DOTALL
)
...
VT_IN_WHERE_PATTERN = re.compile(
    ...
    r'.*?ГДЕ\s+', re.IGNORECASE | re.DOTALL
)
...
VYGRUZKA_EMPTY_CHECK_PATTERN = re.compile(
    r'\.Выгрузить\(\).*\.Количество\(\)\s*=\s*0|\.Выгрузить\(\).*Пустая',
    re.IGNORECASE
)
...
OTMENIT_WITHOUT_RAISE_PATTERN = re.compile(
    r'(ОтменитьТранзакцию\s*\(\s*\))(?!.*ВызватьИсключение)',
    re.IGNORECASE | re.DOTALL
)
...
def rule_no_deep_nesting(lines: list[str], file_path: Path) -> Iterator[Violation]:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if re.search(r'\b(Если|Для|Пока)\s', stripped, re.IGNORECASE) and (
            'Тогда' in stripped or 'Цикл' in stripped
        ):
            depth += 1
            if depth > max_depth:
                yield Violation(
...
ALL_RULES = [
    ...
    # v3.5.0 — из ИТС, Инфостарт, ai_rules_1c, 1c-standards:
    rule_no_otkaz_lozh,
