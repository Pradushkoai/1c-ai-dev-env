...
def main():
    ...
    for config in CONFIGS:
        ...
        print(f"\n--- {name} ---")
        result = subprocess.run([
            sys.executable, '-m', 'src.cli', 'search-code', '--rebuild',
            '--config', name,
        ], cwd='/home/z/my-project/repo_work', capture_output=True, text=True, timeout=300)
        if result.returncode == 0:
            print(f"  ✅ code-search-index построен")
            for line in result.stdout.split('\n')[-5:]:
                if line.strip():
                    print(f"    {line}")
        else:
