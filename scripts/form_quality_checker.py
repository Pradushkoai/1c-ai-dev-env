...
class FormQualityChecker:
    ...
    def _check_form(self, form_data: dict) -> list[FormQualityIssue]:
        ...
        if element_count == 0 and not items:
            issues.append(FormQualityIssue(
                rule_id='FQ001',
                severity='LOW',
                form_name=form_name,
        ...
        self._check_items(items, issues, form_name, parent_name,
                          button_count, no_datapath_count, no_command_count,
                          hidden_count, deep_nesting_count, names, nesting=0)
    ...
    def _check_items(self, items, issues, form_name, parent_name,
                     button_count, no_datapath, no_command,
                     hidden_count, deep_nesting, names, nesting):
        ...
        for item in items:
            ...
            if not data_path and item_type not in ('UsualGroup', 'Pages', 'Page',
                                                    'Button', 'CommandBar',
            ...
            if children:
                self._check_items(children, issues, form_name, parent_name,
                                  button_count, no_datapath, no_command,
                                  hidden_count, deep_nesting, names, nesting + 1)
