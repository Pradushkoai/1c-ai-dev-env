...
- ✅ **v3.14.0**: Замена 5 ролей на чек-лист (проверяемые условия)
...
| Режим | Кто агент | Когда применяется |
|-------|-----------|-------------------|
| **A. MCP-сервер** | Cursor/Claude/VS Code | Внешняя IDE подключается к `mcp_server.py` (8 tools) |
| **B. Прямая работа с LLM** | LLM в чате | Пользователь работает с ассистентом напрямую |
| **C. CLI напрямую** | Пользователь | Пользователь сам запускает `1c-ai` команды |

📖 Подробности: [docs/MCP_INTEGRATION.md](docs/MCP_INTEGRATION.md) — для режима A.

---

## Чек-лист решения задач 1С (режим B)

При работе с ассистентом над задачей 1С — ассистент соблюдает **чек-лист**
из [`runtime/checklist.md`](runtime/checklist.md)
(или `templates/checklist.template.md`).

### 5 фаз чек-листа

1. **Понимание задачи** — уточнить ТЗ, не выдумывать
2. **Контекст** — `solve_context`, `search`, `get_api_reference`
3. **Реализация** — код по ТЗ, методы из контекста
4. **Проверка** — `check_standards` (0 errors), `solve_check --level full` (verdict ∈ {ready, warnings})
5. **Документация** — docstring + примеры

### 3 принципа

1. **Уточни ТЗ** — если неполное, спроси, не выдумывай
2. **Используй репозиторий** — `search`, `solve_context`, `get_api_reference`
3. **Проверь результат** — `check_standards`, `solve_check`

### Как передать в новый чат

В новой сессии скажи ассистенту: «прочитай `runtime/soul.md` и `runtime/checklist.md`» —
и он будет следовать чек-листу.
