...
> **Универсальная среда разработки на 1С с ИИ-ассистентом**: парсинг и анализ метаданных 1С из XML-выгрузок, 54 MCP tools для IDE/LLM, 11 анализаторов BSL-кода (150+ правил), JSON DSL → XML компиляторы (5 типов объектов), работа с расширениями CFE, граф зависимостей метаданных, трассировка СКД, генерация обработок/отчётов/макетов/ролей, **создание внешних обработок .epf с нуля без 1С**, SARIF для GitHub Code Scanning.
...
[![MCP Tools](https://img.shields.io/badge/MCP%20tools-54-blue.svg)](#подключение-к-ide--llm-через-mcp)
...
|------------|--------------|----------|
| **MCP server** (54 tools) | ✅ Stable | Основная функциональность, 1598 тестов |
| **CLI** (19 команд) | ✅ Stable | Все команды работают, тесты проходят |
| **BSL анализаторы** (11 шт, 150+ правил) | ✅ Stable | 56 правил в 5 модулях (Этап 2.1) |
...
Проект включает собственный MCP-сервер с 54 tools для Cursor / Claude Desktop / VS Code / JetBrains.
...
│   │   ├── standards/      56 правил в 5 модулях (Этап 2.1)
...
├── mcp_server.py           MCP-сервер (54 tools)
