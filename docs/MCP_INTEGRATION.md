...
[MCP (Model Context Protocol)](https://modelcontextprotocol.io) — открытый протокол Anthropic, который позволяет IDE/LLM получать доступ к внешним источникам данных и инструментов. 1C AI Development Environment поставляет MCP-сервер, который экспортирует 12 tools для работы с конфигурациями 1С, методами платформы, стандартами разработки и анализом `.bsl` кода.
...
Должны увидеть список из 12 tools. Если команда не найдена — используйте `python3 -m src.cli mcp tools`.
...
После сохранения файла перезапустите Cursor. В панели чата появится иконка молотка (Tools) — нажмите её и убедитесь, что 12 tools от `1c-ai-dev-env` активны.
