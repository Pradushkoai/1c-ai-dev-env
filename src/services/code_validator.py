        ...
        for i, line in enumerate(lines, 1):
            ...
            if re.match(r"^(Процедура|Функция)\s+\w+\s*\(", stripped):
                ...
                if i >= 2:
                    ...
                    if not (
                        prev_line.startswith("&НаСервере")
                        or prev_line.startswith("&НаКлиенте")
                        or prev_line.startswith("&НаКлиентеНаСервере")
                    ):
...
def validate_generated(source_dir: str) -> dict:
    ...
    # 4. Общий вердикт
    total_errors = (
        sum(len(r.get("errors", [])) for r in bsl_results)
        + sum(len(r.get("errors", [])) for r in xml_results)
        + len(structure["errors"])
    )
    total_warnings = (
        sum(len(r.get("warnings", [])) for r in bsl_results)
...
# ============================================================================
# CLI вынесен в scripts/code_validator.py (Этап 1.2, Группа 2)
