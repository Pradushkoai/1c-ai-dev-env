...
class CfeManager:
    ...
    def _borrow_single(
        ...
        if not source_dir.exists() and not source_xml.exists():
            result.warnings.append(
                f"Объект {object_ref} не найден в конфигурации {config_path}"
            )
    ...
    def patch_method(
        ...
        if interceptor_type not in ("Before", "After", "ModificationAndControl"):
            raise ValueError(
                f"Неверный interceptor_type: {interceptor_type}. "
                "Ожидается: Before, After, ModificationAndControl"
            )
    ...
    def diff(
        ...
        for object_type, type_info in TYPE_MAP.items():
            ...
            for xml_file in type_dir.glob("*.xml"):
                ...
                borrowed = self._analyze_borrowed_object(
                    xml_file, object_type, obj_name, config_path
                )
                if borrowed:
