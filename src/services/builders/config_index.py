...
def build_index(config_dir, output_index, config_name):
    ...
    has_xml_files = any(os.path.isdir(os.path.join(config_dir, d)) for d in TYPE_TO_DIR.values())
    if has_xml_files:
    ...
    for type_singular, (ru_name, type_plural) in METADATA_TYPES.items():
        ...
        if type_singular in (
            ...
            for obj in sorted_objs:
                ...
                lines.append(
                    f"| `{obj['name']}` | {syn} | {dims_str} | {res_str} | {attrs_str} | {modules_str} | {forms_str} |"
                )

        elif type_singular == "Catalog":
            lines.append(
                "| Имя | Синоним | Длина кода | Длина наименования | Иерархия | Владельцы | Реквизиты (с типами) | Табличные части | Модули | Формы |"
            )
            lines.append(
                "|-----|---------|------------|--------------------|----------|-----------|-----------------------|-----------------|--------|-------|"
            )
            for obj in sorted_objs:
                ...
                lines.append(
                    f"| `{obj['name']}` | {syn} | {code_len} | {desc_len} | {hier} | {owners} | {attrs_str} | {tabs_str} | {modules_str} | {forms_str} |"
                )
            ...
            for obj in sorted_objs:
                ...
                lines.append(
                    f"| `{obj['name']}` | {syn} | {num_type} | {attrs_str} | {tabs_str} | {modules_str} | {forms_str} |"
                )
        ...
        elif type_singular == "Constant" or type_singular == "CommonForm" or type_singular == "CommonTemplate":
            lines.append("| Имя | Синоним |")
            lines.append("|-----|---------|")
            for obj in sorted_objs:
                syn = obj.get("synonym", "").replace("|", "\\|")
    ...
    subdirs = sorted(
        [d for d in os.listdir(config_dir) if os.path.isdir(os.path.join(config_dir, d)) and not d.startswith(".")]
    )
    for d in subdirs:
        d_path = os.path.join(config_dir, d)
        file_count = sum(1 for f in os.listdir(d_path) if os.path.isfile(os.path.join(d_path, f)))
...
# CLI вынесен в scripts/build_config_index_generic.py (Этап 2.4)
